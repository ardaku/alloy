...
use gelatin::image::{
    self, gif::GifDecoder, png::PngDecoder, AnimationDecoder, ImageFormat,
};
...
pub fn load_svg(path: &std::path::Path) -> Result<image::RgbaImage> {
    ...
    resvg::render(&rtree, usvg::FitTo::Zoom(zoom as f32), tiny_skia::Transform::identity(), pixmap.as_mut())
        .unwrap();
...
impl ImageLoader {
    ...
    fn thread_loop(
        ...
        while running.load(Ordering::Acquire) {
            let request;
            {
                ...
                let focus_test_passed = priority == request.req_id
                    || priority == NON_EXISTENT_REQUEST_ID;
    ...
    pub fn try_recv_prefetched(
        &mut self,
    ) -> std::result::Result<LoadResult, TryRecvError> {
    ...
    fn load_and_send(img_sender: &Sender<LoadResult>, request: LoadRequest) {
        fn try_load_and_send(
            img_sender: &Sender<LoadResult>,
            request: &LoadRequest,
        ) -> Result<()> {
            let metadata = fs::metadata(&request.path)?;
            img_sender
                .send(LoadResult::Start {
