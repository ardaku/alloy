...
use crate::gelatin::{NextUpdate, window::Window};
            ...
            .run(move |event, _event_loop, control_flow| {
                ...
                match event {
                    ...
                    Event::MainEventsCleared => {
                        if !EXIT_REQUESTED.load(Ordering::Relaxed) {
                            let mut should_sleep =
                                !matches!(control_flow, ControlFlow::Poll);
                            for (window_id, window) in windows.iter() {
                                let new_control_flow =
                                    window.main_events_cleared().into();
                                update_control_flow(
                                ...
                                should_sleep =
                                    should_sleep && window.should_sleep();
                    ...
                    Event::RedrawRequested(window_id) => {
                        let new_control_flow =
                            windows.get(&window_id).unwrap().redraw().into();
                        update_control_flow(
                            &mut control_flow_source,
                            window_id,
                            control_flow,
                ...
                if matches!(control_flow, ControlFlow::Poll) {
                    ...
                    let now = std::time::Instant::now();
                    *control_flow =
                        ControlFlow::WaitUntil(now + MAX_SLEEP_DURATION);
                }
            });
    }
}
