...
impl<Dim: PickDimension + 'static> LineLayoutContainer<Dim> {
    ...
    pub fn remove_child(&self, target: Rc<dyn Widget>) {
        ...
        borrowed
            .children
            .retain(|child| target_ptr != widget_data_ptr(child));
        borrowed.render_validity.invalidate();
    }
    ...
    fn layout_aligned_children(
        ...
        for child in alignement_group.iter() {
            ...
            let margins =
                Dim::margin_start(&placement) + Dim::margin_end(&placement);
            ...
            *Dim::rect_pos_mut(widget_available_space) +=
                Dim::rect_size(widget_available_space);
        }
    }
}
...
impl<Dim: PickDimension + 'static> Widget for LineLayoutContainer<Dim> {
        ...
        let mut next_update = NextUpdate::Latest;
        {
            ...
            if borrowed.bg_color[3] > 0.0 {
                context.clear_color(
                    target,
                    borrowed.bg_color,
                    Some(borrowed.drawn_bounds),
                );
            }
            for child in borrowed.children.iter() {
                next_update =
                    next_update.aggregate(child.draw(target, context)?);
            }
        }
        Ok(next_update)
    }
    ...
    fn layout(&self, mut total_available_space: LogicalRect) {
        ...
        let center_start_pos = (center_pos - center_max_size * 0.5)
            .max(Dim::rect_pos(&widget_available_space));
        *Dim::rect_pos_mut(&mut widget_available_space) = center_start_pos;
        ...
        let end_start_pos = (end_end_pos - end_max_size)
            .max(Dim::rect_pos(&widget_available_space));
