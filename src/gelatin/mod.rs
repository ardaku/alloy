...
pub trait WidgetData {
    ...
    fn apply_horizontal_alignement(
        ...
        match self.placement().horizontal_align {
            ...
            Alignment::Center => {
                ...
                self.drawn_bounds().pos.vec.x += self.placement().margin_left
                    + space_between_margins * 0.5
                    - width * 0.5;
            }
            Alignment::End => {
                self.drawn_bounds().pos.vec.x = available_space.right()
                    - (self.placement().margin_right + width);
            }
        }
    }
    fn apply_vertical_alignement(
        ...
        match self.placement().vertical_align {
            ...
            Alignment::End => {
                self.drawn_bounds().pos.vec.y = available_space.bottom()
                    - (self.placement().margin_bottom + height);
            }
        }
    }
        ...
        *self.drawn_bounds() = available_space;
        match self.placement().width {
            ...
            Length::Stretch { min, max } => {
                ...
                width -= self.placement().margin_left
                    + self.placement().margin_right;
        ...
        match self.placement().height {
            ...
            Length::Stretch { min, max } => {
                ...
                height -= self.placement().margin_top
                    + self.placement().margin_bottom;
