...
impl Widget for Slider {
    fn draw(
    ...
    ) -> Result<NextUpdate, WidgetError> {
        {
            ...
            let aligned_bounds = borrowed
                .drawn_bounds
                .align_to_pixels(context.dpi_scale_factor);
            ...
            let value_ratio =
                (borrowed.value as f32 + 0.5) / (borrowed.steps as f32);
            ...
            let mut transform =
                Matrix4::from_nonuniform_scale(1.0, size.y, 1.0);
    ...
    fn handle_event(&self, event: &Event) {
        ...
        let check_value_change = || {
            ...
            let on_value_change;
            {
                ...
                if borrowed.click {
                    let prev_value = borrowed.value;
                    let relative_cursor_x = event.cursor_pos.vec.x
