...
impl PlaybackManager {
    ...
    pub fn start_random_presentation(&mut self) {
        self.folder_player
            .start_random_presentation(&mut self.image_cache);
        //self.playback_start_time = Instant::now();
        //self.frame_count_since_playback_start = 0;
...
impl<P: Playback> ImgSequencePlayer<P> {
    ...
    pub fn start_random_presentation(
        &mut self,
        image_cache: &mut ImageCache,
    ) -> bool {
        self.last_frame_change_time = Instant::now();
    ...
    pub fn update_image(
        ...
        if self.playback_state == PlaybackState::Paused {
            ...
            let elapsed_nanos = elapsed.as_secs() * NANOS_PER_SEC
                + elapsed.subsec_nanos() as u64;
            let elapsed_nanos =
                elapsed_nanos as i64 + self.frametime_drift_offset;
            ...
            let frame_step = if frame_delta_time_nanos > 0 {
            ...
            if frame_step > 0 {
                ...
                if elapsed_nanos
                    > (frame_delta_time_nanos as f32 * BUISY_WAIT_THRESHOLD)
                        as i64
                {
                    ...
                    match self.playback_state {
                        PlaybackState::RandomPresent => {
                            if let Some(&last) =
                                self.present_remaining.iter().last()
                            {
        ...
        let load_result = match load_request {
            ...
            LoadRequest::LoadPrevious => {
                Some(P::load_prev(image_cache, display))
            }
            LoadRequest::FilePath(file_path) => Some(
                P::load_path(image_cache, display, &file_path)
                    .map(|x| (x, file_path)),
            ),
            LoadRequest::LoadAtIndex(index) => {
                Some(P::load_at_index(image_cache, display, index))
            }
            LoadRequest::Jump(jump_count) => {
                Some(P::load_jump(image_cache, display, jump_count))
            }
    ...
    fn fill_present_remainig_with_random(
        &mut self,
        image_cache: &mut ImageCache,
    ) -> bool {
